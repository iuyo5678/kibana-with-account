module.exports = async (kbnServer, server, config) => {
  ...
  let optimizer = new FsOptimizer({
    env: bundles.env,
    bundles: bundles,
    profile: config.get('optimize.profile'),  // false
    urlBasePath: config.get('server.basePath'),       // ""
    sourceMaps: config.get('optimize.sourceMaps'),   // false
    unsafeCache: config.get('optimize.unsafeCache'), // /[\/\\](node_modules|bower_components)[\/\\]/
  });
